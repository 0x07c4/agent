...
### Summary
用户要求“定期收集”这类周期任务时，不能只补文档流程；必须落到可重复执行的脚本、系统定时器、cron、GitHub Actions 或等价调度入口。

### Details
用户明确纠正：只增加说明文档并不能实现定期收集。正确交付应至少包含：
- 一条可手动执行、可幂等重复运行的入口命令
- 一个真实调度机制，例如 `systemd --user timer`
- 触发后的结构化输出位置和后续人工处理清单

### Suggested Action
- 看到“定期、周期、每周、自动提醒、定时收集”时，默认检查是否需要调度器
- 文档只能作为使用说明，不能代替实际自动化入口
- 涉及质量筛选或隐私判断时，自动化负责触发和排队，最终收录保留 human-in-the-loop

### Metadata
- Source: user correction / conversation
- Tags: automation, scheduler, recurring-work, human-in-the-loop
...
## 2026-04-26 · Pencil 与 OpenPencil 命名不要混用

- 用户提到 `pencil` 时优先理解为 Pencil.dev（High Agency 的产品，CLI 包为 `@pencil.dev/cli`，命令 `pencil`，文件格式 `.pen`），不是泛称，也不是仓库里的 `pencil.c`。
- 当前本机 Solo 实践使用的是 OpenPencil（`@zseven-w/openpencil`，命令 `op`，文件格式 `.op`），它是 Pencil.dev 的开源替代/竞争工具，不是同一产品。
- 讨论设计工具链时要明确写 `Pencil.dev` / `OpenPencil` / `op CLI`，避免简称 `pencil` 造成误判。
- 另外存在 `https://openpencil.dev/` 对应的 `open-pencil/open-pencil` 项目（命令/包倾向 `open-pencil` / `@open-pencil/*`，支持 `.fig`/`.pen`），这和 `ZSeven-W/openpencil` 也是不同项目；看到 `openpencil.dev` 时不要默认当成 `@zseven-w/openpencil`。

## 2026-04-26
- Solo UI：当 runtime snapshot/IPC 卡住或缺失时，不要让 loading/状态事件覆盖真实结果；优先从 session.messages 投影 assistant response，并用 timeline 显示 user/assistant 消息，避免“回复生成完成”这类状态被误读成结果。

## 2026-04-26
- Solo runtime UI：不要用“首个/first event”这类偷懒语义描述运行状态；应按 current turn 边界表达“等待当前 turn 的新进展/结果”，并避免用 session 里的旧 assistant 回复冒充当前 run 输出。

## 2026-04-26 - Solo 迭代前需要校准同类 OSS 项目
用户指出 `multica-ai/multica` 已经覆盖 managed agents platform 方向。后续推进 Solo 产品设计时，不能只在当前 UI 局部迭代；应先做竞品/参考实现校准，明确 Solo 是否走 local-first personal control plane、单人工作台、桌面态观测与决策确认等差异化边界，避免重复构建已有团队型 managed agents 平台。

## 2026-04-26 - Terminal agent slash command UX
- Terminal-native agent CLI 不能只提供 readline/Tab 补全；用户输入 `/` 后应立即显示已有 slash commands，行为要接近 Codex/Claude Code 的 composer。
- 没有 `prompt_toolkit` 等 UI 依赖时，也应提供轻量 TTY fallback，而不是让体验退回普通 `input()`。

## 2026-04-27 - mypy via pipx
- 用户机器上的 `mypy` 通过 pipx 安装并暴露为 PATH 可执行文件；不要只用 `python -m mypy` 判断 mypy 是否可用。
- 在 Python 项目里优先运行 `mypy <paths>`；只有需要验证当前虚拟环境内安装状态时才使用 `python -m mypy`。

## 2026-04-27 - Terminal agent @path UX
- 当 terminal-native agent 支持 `@path` 作为显式工作区上下文入口时，`@` 必须像 `/` 一样立即触发候选提示。
- 精确替换类 edit proposal 失败时，不要只打印内部错误；要说明 diff/apply 不可用，并提示用最新 `@path` 上下文重新生成或 `/reject` 当前 proposal。

## 2026-05-02 - GitHub 新建仓库工作流
- 当前 GitHub MCP/连接器能读写已有仓库内容、issue、PR 等，但没有暴露“创建新仓库”能力；新建 GitHub repo 仍需走 `gh repo create` 或用户在网页创建。
- 沙箱内执行 `gh auth login` / `gh repo create` 会因为 GitHub 网络访问失败，需要按策略用 `require_escalated` 重跑。
- 如果 `gh auth status` 显示本地 token invalid，即使 GitHub MCP 账号可用，也要先刷新 GitHub CLI 登录态，再创建 repo 和 push。

## 2026-05-03 - Codex / DeepSeek / local model 分工
- 用户未来的 vibe coding 组合是 ChatGPT Plus/Codex + DeepSeek V4 + local model，但 Codex 不应默认作为实现器；它应主要停留在 plan/design/review 层。
- DeepSeek 是主力实现和草稿生成层；本地模型只是辅助层。
- DeepSeek 或本地模型产出的任何代码 diff，都需要 Codex review 后才进入用户审批。
- 不要把 Ollama 当作该工作流的默认 local model 路径；本地层按 runner-neutral 的 OpenAI-compatible endpoint 处理，当前可由 LM Studio 的 `qwen3.5-9b` 提供。
- 在 cocoa/Solo 还没有一等 plan/handoff/review item 之前，如果 Codex 和 OpenCode/DeepSeek 分别在两个窗口里运行，就用 repo 内 Markdown handoff 文件作为共享 artifact：Codex 写 plan/review findings，DeepSeek 读 plan 实现并回填测试结果，最终仍由用户审批。

## 2026-05-03 - RTK Codex 集成
- 用户已安装 `rtk 0.38.0` 到 `~/.local/bin/rtk`，并要求按官方文档集成到 Codex。
- `rtk init -g --codex` 会在 `~/.codex/AGENTS.md` 中追加 `@~/.codex/RTK.md`，同时生成 `~/.codex/RTK.md`；Codex 集成是 AGENTS 指令方式，不是强制 hook，所以 `rtk gain` 仍可能提示 no hook installed。
- 在 Codex 沙箱中 `rtk gain` 如果报 `Failed to initialize tracking database: unable to open database file`，把 `~/.config/rtk/config.toml` 的 `[tracking].database_path` 指向 `~/.codex/memories/rtk/history.db`，并确保目录存在。
- RTK 指令不应写成“所有 shell command 都加 rtk”；应只包装高噪声命令，避免把 `test -f`、`pwd`、`mkdir`、`command -v`、短 `sed` 这类精确/简短命令变成错误的 `rtk test ...`。
- agent 配置同步脚本需要包含 `RTK.md`，同时排除 `memories/rtk/`，避免把 RTK tracking DB 同步进 `agent.git`。
